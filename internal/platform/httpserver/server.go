...
func NewServer(cfg config.Config, logger *slog.Logger, pool *pgxpool.Pool) *http.Server {
	...
	if pool != nil {
		...
		mux.Handle("POST /v1/withdrawals", requireSession(authService)(requireStepUp(authService, "withdrawal_created", http.HandlerFunc(withdrawalHandler.Request))))
		mux.Handle("GET /v1/withdrawals", requireSession(authService)(http.HandlerFunc(withdrawalHandler.History)))
		mux.Handle("POST /v1/wallet/withdrawals", requireSession(authService)(requireStepUp(authService, "withdrawal_created", http.HandlerFunc(withdrawalHandler.Request))))
