...
func NewServer(cfg config.Config, logger *slog.Logger, pool *pgxpool.Pool) *http.Server {
	...
	if pool != nil {
		...
		accountHandler := NewAccountHandler(accounts.NewService(data, cfg.VerificationPepper, cfg.VerificationEncryptionKey), logger)
		mux.HandleFunc("POST /v1/auth/register", accountHandler.Register)
		authService := auth.NewService(data, cfg.SessionTTL, cfg.TOTPEncryptionKey)
		authHandler := NewAuthHandler(authService, logger, cfg.Environment != "development")
		mux.HandleFunc("POST /v1/auth/login", authHandler.Login)
		mux.HandleFunc("POST /v1/auth/totp/verify", authHandler.VerifyTOTPLogin)
		emailVerificationHandler := NewEmailVerificationHandler(auth.NewEmailVerificationService(data, cfg.VerificationPepper, cfg.VerificationEncryptionKey), logger)
