...
func (m *Manager) UpdateUserPreferences(ctx context.Context, userID, displayName, currency, secondaryAsset, language, theme string) (UserProfile, error) {
	...
	if _, err = tx.Exec(ctx, `INSERT INTO audit_events(actor_id,actor_type,action,resource_type,resource_id,metadata) VALUES($1::uuid,'user','user.preferences_updated','user',$2::text,'{}')`, userID, userID); err != nil {
		return UserProfile{}, err
	}
...
func (m *Manager) ApprovedWithdrawals(ctx context.Context, provider string, limit int) ([]WithdrawalForCustody, error) {
	...
	rows, err := m.pool.Query(ctx, `SELECT w.id::text,w.user_id::text,a.id::text,c.external_vault_id,a.custody_asset_id,a.network,w.destination_address,w.destination_tag,w.amount_atomic::text,a.decimals
		FROM withdrawals w
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.UserID, &item.AssetID, &item.SourceVaultID, &item.CustodyAssetID, &item.Network, &item.DestinationAddress, &item.DestinationTag, &item.AmountAtomic, &item.AssetDecimals); err != nil {
			return nil, err
		}
		items = append(items, item)
	}
