...
func (store *PostgresStore) ListConfigs(ctx context.Context) ([]Config, error) {
	rows, err := store.pool.Query(ctx, `SELECT c.pair,c.enabled,p.price_scale,p.price_tick_atomic::text,c.spread_bps,c.quantity_atomic::text,
		...
		FROM market_maker_configs c JOIN trading_pairs p ON p.symbol=c.pair
	...
	for rows.Next() {
		...
		if err = rows.Scan(&config.Pair, &config.Enabled, &config.PriceScale, &config.PriceTickAtomic, &config.SpreadBPS, &config.QuantityAtomic,
