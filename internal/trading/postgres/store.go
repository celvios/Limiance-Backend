...
func (store *Store) CreateOrder(ctx context.Context, input trading.CreateOrderCommand) (trading.Order, error) {
	...
	err = tx.QueryRow(ctx, `
		...
		RETURNING id::text,user_id::text,account_id::text,pair,side,type,COALESCE(price,0)::text,COALESCE(trigger_price,0)::text,
		...
		input.Price, input.Quantity, input.TimeInForce, input.PostOnly, input.ReduceOnly,
		input.FeeTier, input.IdempotencyKey, input.RequestHash[:], initialStatus, input.TriggerPrice, input.TriggerDirection).Scan(orderScanTargets(&order)...)
	if err != nil {
		return trading.Order{}, err
	}
	...
	if _, err := tx.Exec(ctx, `INSERT INTO outbox_events (event_type,aggregate_type,aggregate_id,payload) VALUES ('order.pending','order',$1,$2::jsonb)`, input.OrderID, payload); err != nil {
		return trading.Order{}, err
	}
	if err := tx.Commit(ctx); err != nil {
		return trading.Order{}, err
...
func (store *Store) ApplyEngineStatus(ctx context.Context, orderID string, event protocol.OrderStatusEvent) (trading.Order, error) {
	...
	var currentStatus, quantity, holdJournalID, holdAssetID, holdAmount, accountID string
	err = tx.QueryRow(ctx, `
		SELECT status,quantity::text,COALESCE(hold_journal_id::text,''),COALESCE(hold_asset_id::text,''),
		       COALESCE(hold_amount_atomic,0)::text,account_id::text
		FROM orders WHERE id=$1 FOR UPDATE`, orderID).
		Scan(&currentStatus, &quantity, &holdJournalID, &holdAssetID, &holdAmount, &accountID)
	if err != nil {
		return trading.Order{}, err
	}
	...
	if currentStatus != nextStatus || currentStatus != "FILLED" && currentStatus != "CANCELED" && currentStatus != "REJECTED" {
		if _, err := tx.Exec(ctx, `
			...
			WHERE id=$1`, orderID, nextStatus, event.FilledQuantity, event.RemainingQuantity, event.AvgPrice, event.SequenceID); err != nil {
			return trading.Order{}, err
		}
		...
		if nextStatus == "REJECTED" && event.FilledQuantity == 0 && holdJournalID != "" {
			var releaseJournalID string
			if err := tx.QueryRow(ctx, `INSERT INTO journals (idempotency_key,reference_type,reference_id) VALUES ($1,'order_release',$2) RETURNING id::text`, "order-release:"+orderID, orderID).Scan(&releaseJournalID); err != nil {
				return trading.Order{}, err
			}
			if _, err := tx.Exec(ctx, `
				INSERT INTO postings (journal_id,account_id,asset_id,bucket,direction,amount_atomic) VALUES
				($1,$2,$3,'held','debit',$4::numeric),
				($1,$2,$3,'available','credit',$4::numeric)`, releaseJournalID, accountID, holdAssetID, holdAmount); err != nil {
				return trading.Order{}, err
			}
		}
...
func (store *Store) RecordDispatchFailure(ctx context.Context, orderID, reason string) error {
	...
	_, err := store.pool.Exec(ctx, `UPDATE engine_commands SET attempts=attempts+1,last_error=$2,updated_at=now() WHERE order_id=$1 AND status='pending'`, orderID, reason)
	return err
}
...
func findByIdempotencyKey(ctx context.Context, tx pgx.Tx, userID, key string) (trading.Order, []byte, bool, error) {
	...
	err := tx.QueryRow(ctx, `
		SELECT o.id::text,o.user_id::text,o.account_id::text,o.pair,o.side,o.type,COALESCE(o.price,0)::text,COALESCE(o.trigger_price,0)::text,
