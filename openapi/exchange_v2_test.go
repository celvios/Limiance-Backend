...
func TestExchangeV2ContractParsesAndContainsRequiredSurface(t *testing.T) {
	...
	text := string(payload)
	for _, required := range []string{"openapi: 3.1.0", "/orders:", "/market/orderbook/{pair}:", "/market/klines/{pair}:", "/ws:", "X-API-Nonce", "request_id", "Idempotency-Key", "STOP_MARKET", "TAKE_PROFIT_LIMIT"} {
